# -*- coding: utf-8 -*-
"""
平台健康检查 - 六维度检查函数（从 healthz.py 拆分）

维度：database / redis / thread_pool / task_queue / event_loop / downstream
请求耗时采样数据源（_req_times/_req_recent）仍由 healthz.py 维护，此处引用。
"""
import logging
import threading
import time
...
def check_event_loop():
    ...
    times = sorted(healthz._req_times)
    p95 = healthz._pct(times, 0.95)
    p99 = healthz._pct(times, 0.99)
    metrics = {
        'samples': len(times),
    ...
    metrics['recent'] = list(healthz._req_recent)[::-1][:30]
    if len(times) < 5:
